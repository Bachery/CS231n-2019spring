...
#         pass
        out1 = X.dot(W1) + b1             # (N, H)
        out1_relu = np.maximum(0, out1)   # ReLU
        scores = out1_relu.dot(W2) + b2   # (N, C)
...
#         log = np.multiply(np.log(probs), y_label)
#         loss = (-1) * np.sum(log) / N
