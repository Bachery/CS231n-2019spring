...
#     pass
    ...
    x_norm_T = (x.T - sample_mean) / np.sqrt(sample_var + eps)
    x_norm = x_norm_T.T
    out = x_norm * gamma + beta
    cache = (x, x_norm, gamma, sample_mean, sample_var, eps)
...
#     pass
    ...
    dv = ((x.T - sample_mean) * -0.5 * (sample_var+eps)**-1.5 * dx_norm).sum(axis=0)
    dm = (dx_norm * -1 * (sample_var+eps)**-0.5).sum(axis=0) + (dv * (x.T-sample_mean) * -2 / N).sum(axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    Input:
    - x: Input data of shape (N, C, H, W)         输入  (数量，通道，高，宽)
    - w: Filter weights of shape (F, C, HH, WW)   卷积核 (数量，通道，高，宽)
    - b: Biases, of shape (F,)
    - conv_param: A dictionary with the following keys:
      - 'stride': The number of pixels between adjacent receptive fields in the   stride参数：S，决定了卷积核每次移动多少
        horizontal and vertical directions.
      - 'pad': The number of pixels that will be used to zero-pad the input.      pad参数: P，决定了你要在四周pad多少
...
#     print(x_pad[0,0,:,:])
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            i_s = i * stride
            j_s = j * stride
            x_padded_mask = x_pad[:, :, i_s:i_s+HH, j_s:j_s+WW] # (:, :, HH, WW)
            for k in range(F):
                # 卷积
                out[:, k, i, j] = np.sum(x_padded_mask * w[k,:,:,:], axis=(1,2,3))
    
    out += b[None, :, None, None] # 加上bias，None相当于numpy.newaxis
...
#     pass
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for n in range(N):
                dx_pad[n, :, i_s:i_s+HH, j_s:j_s+WW] += np.sum( (dout[n,:,i,j])[:,None,None,None] * w, axis=0 )
...
#     pass
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            i_s = i * stride
            j_s = j * stride
            x_padded_mask = x[:, :, i_s:i_s+pool_height, j_s:j_s+pool_width] # (:, :, H, W)
            # 找出最大值
...
#     pass
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            dx[:, :, i_s:i_s+pool_height, j_s:j_s+pool_width] += temp_binary_mask * (dout[:,:,i,j])[:,:,None,None]
...
#     pass
    N, C, H, W = x.shape
    x_group = np.reshape(x, (N, G, C//G, H, W))           # 按 G 将 C 分组
...
def svm_loss(x, y):
    ...
    # correct_class_scores[:, np.newaxis]      # (N, 1)
    margins = np.maximum(0, x - correct_class_scores[:, np.newaxis] + 1.0) # (N, C)
    margins[np.arange(N), y] = 0               # 正确分类的loss置0
...
def softmax_loss(x, y):
    ...
    - dx: Gradient of the loss with respect to x
    """
    shifted_logits = x - np.max(x, axis=1, keepdims=True)     # (N, C) - (N, 1)
    Z = np.sum(np.exp(shifted_logits), axis=1, keepdims=True) # (N, 1)
    log_probs = shifted_logits - np.log(Z)                    # (N, C) - (N, 1)
